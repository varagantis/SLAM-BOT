...
#include <lcmtypes/particles_converged_t.hpp>
#include <vector>
...
public:
    ...
    */
    void initializeFilterAtPose(const pose_xyt_t& pose);
        ...
        /**
    * initializeFilterUniformly initializes the particle filter with the samples distributed uniformly on
    * a grid./
    *
    * \param    pose            Initial pose of the robot
    ...
    */
    ...
    /**
    * updateFilterLocalizationUnknown increments the state estimated by the particle filter. The filter update uses the most recent
    * odometry estimate and laser scan along with the occupancy grid map to estimate the new pose of the robot.
    *
    * \param    odometry        Calculated odometry at the time of the final ray in the laser scan
    * \param    laser           Most recent laser scan of the environment
    ...
    * \return   Estimated robot pose.
    */
    pose_xyt_t updateFilterLocalizationUnknown(const pose_xyt_t&      odometry,
                            const lidar_t& laser,
                            const OccupancyGrid&   map,
...
private:
    ...
    pose_xyt_t resamplePose_;              // Last pose a resample was taken at
    ...
    bool kLocalizationConverged_; // Whether the localization is converged for unknown localization 
    ...
    bool localizationConverged(const std::vector<particle_t>& posterior);
};
