...
OccupancyGridSLAM::OccupancyGridSLAM(int         numParticles,
                                     ...
                                     bool actionOnlyMode,
                                     bool convergeOnlyMode,
                                     const std::string escapeMap,
...
, escape_filter_(numParticles)
, map_(10.0f, 10.0f, 0.05f) //30,30,0.1  // create a 10m x 10m grid with 0.05m cells
, escape_map_(10.0f, 10.0f, 0.05f) //30,30,0.1  // create a 10m x 10m grid with 0.05m cells
...
, converged_({0, false})
{
    ...
    else if(escapeMap.length() > 0)
    {
        mode_ = escape;
        haveMap_ = map_.loadFromFile(escapeMap);
        assert(haveMap_);
    }
...
void OccupancyGridSLAM::runSLAM(void)
{
    while(true)
    {
        ...
        if(isReadyToUpdate())
        {
            ...
            runSLAMIteration();
            // std::cout << "Actually running slam";
        }
...
{
    // std::cout << "Actually running slam iteration\n";
    ...
    {
        // std::cout << "Actually updating slam\n";
        updateLocalization();
        updateMap();
    }
    else 
...
void OccupancyGridSLAM::initializePosesIfNeeded(void)
{
    ...
    if(!haveInitializedPoses_)
    {
        ...
        if (mode_ != escape){
            ...
            filter_.initializeFilterUniformly(previousPose_, map_);
        }   
    }
...
void OccupancyGridSLAM::updateLocalization(void)
{
    ...
    if(haveMap_ && (mode_ != mapping_only))
    {
        ...
        else if(mode_ == escape){
            currentPose_  = filter_.updateFilterLocalizationUnknown(currentOdometry_, currentScan_, map_, &converged_);
        }
        else{
            currentPose_  = filter_.updateFilter(currentOdometry_, currentScan_, map_); // /takes in current map, takes in current odeometry, and current slam 
        }
...
{
    if(mode_ != localization_only && mode_ != action_only && mode_ != escape)
    {
        // std::cout << "Actually updating map\n";
    ...
    {
        // std::cout << "Actually publishing map\n";

        occupancy_grid_t mapMessage;
        mapMessage = map_.toLCM();

        // occupancy_grid_t mapMessage = map_.toLCM();
