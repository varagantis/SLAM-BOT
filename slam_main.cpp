...
int main(int argc, char** argv)
{
    ...
    const char* kEscapeMazeArg = "escape-maze";
    const char* kConvergeOnlyArg = "converge-only";
    ...
    getopt_add_bool(gopt, '\0', kConvergeOnlyArg, 0, "Flag indicating if converge-only mode should be run");
    getopt_add_string(gopt, '\0', kEscapeMazeArg, "", "Flag indicating if localization unknown mode should be run");
    ...
    bool convergeOnly = getopt_get_bool(gopt, kConvergeOnlyArg);
    std::string escapeMap = getopt_get_string(gopt, kEscapeMazeArg);
