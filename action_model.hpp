...
public:
    ...
    */
    bool updateAction(const pose_xyt_t& odometry); // updates the distruction basied on the rotation and translation of the robot
...
private:
    ...
    double rot2_;
    bool moved_;
    bool initialized_; //check to see if we moved 
    int64_t utime_; //time

    double rot1Std_; // first std in rotation
    double transStd_; // 1st std of translation 
    double rot2Std_; // 2nd std in rotation 2

    std::mt19937 numberGenerator_; // creates random noise generator--> indude in # random 
